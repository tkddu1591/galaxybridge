...
> **현재 0.2.0은 검증 중이며 공개 설치는 아직 열리지 않았습니다.** 아래는 준비된 설치 명령입니다. 검증된 릴리스가 공개되기 전에는 관리자 인증 없이 중단합니다. 실제 폰에서 DHCP 연결은 확인했지만, 수정한 작업자의 연결 유지와 USB 분리 후 자동 복귀 검증이 남아 있습니다.
